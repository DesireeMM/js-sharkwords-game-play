...
const handleCorrectGuess = (letter) => {
  ...
  const letterDivList = document.querySelectorAll(`.${letter}`);
  
  for (const div of letterDivList) {
  ...
  const wordLengthList = document.querySelectorAll('.letter-box');
  console.log(numRight);
  console.log(wordLengthList.length);

  if (numRight === wordLengthList.length) {
    for (button of document.querySelectorAll('button')) {
      disableLetterButton(button);
    }

    const winLink = document.querySelector('#win');
    winLink.style.display='block';
  }
};
...
const handleWrongGuess = () => {
  ...
  if (numWrong === 5) {
    ...
    //make an event to change the a with id play-again to display:""
